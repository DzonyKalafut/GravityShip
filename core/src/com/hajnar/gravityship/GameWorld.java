...
public class GameWorld {
    ...
    public void loadWorld(int worldNumber) {
        try {
            ...
            Assets.objectBodiesLoader = new BodyEditorLoader(Gdx.files.internal("data/gravityShipObjects.obj2d"));
            landingZones = new ArrayList<LandingZone>();
            canons = new ArrayList<Canon>();
            bullets = new ArrayList<Bullet>();
            blackHoles = new ArrayList<BlackHole>();
    ...
    public void updateBlackHoles(float delta) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            gravityVector.nor();
            gravityVector.scl(gravityForce);
            player.getBody().applyForceToCenter(gravityVector, true);
        }
    }
        ...
        if (player.getState() != Player.SHIP_STATE_DEAD) {
            if (player.isThrustEnabled()) {
                if (player.getFuelAmmount() > 0) {
                    ...
                    player.getBody().applyForceToCenter(tmpVector.set(x, y), true);
...
//				player.getBody().applyAngularImpulse(-0.13f*player.getRotationRatio());
                player.getBody().applyAngularImpulse(-7.8f * delta * player.getRotationRatio(), true);
