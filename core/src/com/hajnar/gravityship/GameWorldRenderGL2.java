...
public class GameWorldRenderGL2 {
    ...
    public void renderAndroid(float delta) {
        if (world.getState() == GameWorld.WORLD_RUNNING) {
            ...
            parallaxCamera.translate(cameraTranslation.scl(0.1f));
            parallaxCamera2.translate(cameraTranslation.scl(0.3f));
        }
        camera.update();
    ...
    public void renderTerrain() {
        ...
        for (int i = 0; i < world.getTerrain().getMeshes().size(); i++) {
            if (camera.frustum.boundsInFrustum(world.getTerrain().getBoundingBoxes().get(i)))
                (world.getTerrain().meshes.get(i)).render(terrainShader, 5);
        }
        terrainShader.end();
    }
    ...
    public void renderLandingZones() {
        ...
        for (int i = 0; i < len; i++) {
            ...
            spriteCenter.scl(Helper.BOX_TO_WORLD);
